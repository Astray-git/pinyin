...
function segment(hans) {
  try {
    ...
    console.error();
    console.error("    Segment need @node-rs/jieba, please run '$ npm install @node-rs/jieba'.");
    console.error("    分词需要使用 @node-rs/jieba 模块，请运行 '$ npm install @node-rs/jieba' 并确保安装完成。");
    console.error();
    throw ex;
  }
  // 词语拼音库。
  PHRASES_DICT = PHRASES_DICT || require("../data/phrases-dict");
  return jieba.cut(hans, false);
}
